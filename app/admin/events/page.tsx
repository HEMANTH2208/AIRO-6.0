...
const emptyEvent = (): Partial<Event> => ({
  ...
  max_team_size: 4,
  status: "active",
  max_other_college_participants: 60,
  coordinator_name: "",
  coordinator_photo: "",
  coordinator_contact: "",
});
              ...
              <tbody>
                {events.map((ev) => (
                  <tr key={ev.id}>
                    ...
                    <td>
                      <code style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>{ev.slug}</code>
                    </td>
                    <td style={{ fontSize: "0.85rem" }}>{ev.duration}</td>
                    <td>
                      {ev.min_team_size === ev.max_team_size
                        ? ev.min_team_size
                        : `${ev.min_team_size}–${ev.max_team_size}`}
                    </td>
                    <td style={{ textAlign: "center", fontWeight: 700 }}>
                      {ev.max_other_college_participants}
                    </td>
                    <td>
                    ...
                    <td>
                      <div style={{ display: "flex", gap: "0.5rem" }}>
                        ...
                        <button className="btn btn-danger btn-sm" onClick={() => setDeleteConfirm(ev.id)}>
                          Delete
                        </button>
                      </div>
                    </td>
            ...
            <div className="modal" style={{ maxWidth: "600px" }} onClick={(e) => e.stopPropagation()}>
              ...
              <div style={{ maxHeight: "70vh", overflowY: "auto", paddingRight: "0.5rem" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                  ...
                  <div className="form-group">
                    <label className="form-label">Max Other College Members Limit</label>
                    <input
                      type="number"
                      className="form-control"
                      value={editing.max_other_college_participants || 60}
                      onChange={(e) =>
                        setEditing({
                          ...editing,
                          max_other_college_participants: Number(e.target.value),
                        })
                      }
                    />
                  </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                  <div className="form-group">
                    ...
                    <input
                      type="number"
                      className="form-control"
                      value={editing.min_team_size || 2}
                      onChange={(e) => setEditing({ ...editing, min_team_size: Number(e.target.value) })}
                    />
                  </div>
                  <div className="form-group">
                    ...
                    <input
                      type="number"
                      className="form-control"
                      value={editing.max_team_size || 4}
                      onChange={(e) => setEditing({ ...editing, max_team_size: Number(e.target.value) })}
                    />
                  </div>
                </div>

                {/* Coordinator Details Section */}
                <div style={{ background: "var(--bg-surface)", padding: "1rem", borderRadius: "var(--radius-md)", border: "1px dashed var(--border)", marginBottom: "1rem" }}>
                  <div style={{ fontSize: "0.85rem", fontWeight: 700, marginBottom: "0.75rem", color: "var(--primary-light)" }}>📞 Event Coordinator (Feature)</div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                    <div className="form-group" style={{ marginBottom: 0 }}>
                      ...
                      <input
                        ...
                        value={editing.coordinator_contact || ""}
                        onChange={(e) => setEditing({ ...editing, coordinator_contact: e.target.value })}
                      />
                    </div>
                    <div className="form-group" style={{ marginBottom: 0 }}>
                      ...
                      <input
                        className="form-control"
                        placeholder="/images/coordinators/john.jpg"
                        value={editing.coordinator_photo || ""}
                        onChange={(e) => setEditing({ ...editing, coordinator_photo: e.target.value })}
                      />
                    </div>
                  </div>
                </div>
          ...
          <div className="modal-overlay">
            <div className="modal" style={{ maxWidth: "380px" }}>
              ...
              <p style={{ marginBottom: "1.5rem" }}>
                Are you sure you want to delete this event? All associated registrations will also be deleted. This cannot be undone.
              </p>
